document.addEventListener('DOMContentLoaded', () => {
    ...
    for (let i = 0; i < game.attemptsNumber; i++) {
        ...
        updateProgress(((100 / game.attemptsNumber) / keyCount), 'Эмуляция игры...');
        ...
        document.querySelectorAll('.copyKeyBtn').forEach(button => {
            button.addEventListener('click', (event) => {
                ...
                navigator.clipboard.writeText(key).then(() => {
                    copyStatus.innerText = `Скопирован ${key}`;
                    setTimeout(() => {
    ...
    document.getElementById('creatorChannelBtn').addEventListener('click', () => {
        window.open('https://t.me/cryptonbazachat', '_blank');
    });

    telegramChannelBtn.addEventListener('click', () => {
        window.open('https://t.me/cryptonbaza', '_blank');
    });
    ...
    document.getElementById('ShowKeysBtn').addEventListener('click', () => {
        ...
        if (codesGeneratedToday.length > 0) {
            ...
            generatedCodesList.innerHTML = '<li>Пока еще нет кодов.</li>';
        }
